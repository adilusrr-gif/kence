...
const NAV_BASE = [
  ...
  { path: '/convert',      label: 'Конвертер',   icon: '🔄' },
...
export default function App() {
  ...
  const saveName = () => {
    setEditName(false)
    const val = appName.trim() || 'KENCE.ai'
    setAppName(val)
    localStorage.setItem('appName', val)
  }
  const saveEmoji = () => {
    setEditEmoji(false)
    const val = appEmoji.trim() || 'K'
    setAppEmoji(val)
    localStorage.setItem('appEmoji', val)
  }
        ...
        <AnimatePresence>
          {!collapsed && (
            ...
            >
              ...
              <button className="user-badge__logout" onClick={handleLogout} title="Выйти">⏏</button>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.aside>
