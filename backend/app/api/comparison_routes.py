...
async def compare_semantic(request: Request, session_id: str = Query(...), user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=413, detail=f"Документ слишком большой: {e.chars:,} символов (максимум {e.limit:,}). Разделите файл и загрузите частями.")
...
async def compare_technical(request: Request, session_id: str = Query(...), user: dict = Depends(get_current_user)):
    ...
    prompt_user_token = ai_settings_service.set_current_username(user.get("username"))
    try:
